...
Answer: <The spoken answer on this same line. Short sentences starting with 'I'. Max 10 words per sentence. Use as many sentences as needed. Structure: (1) First sentence frames the approach as addressing the business outcome. (2) Signpost sentence names the 2-3 strategies from the cues below. (3) One claim sentence per strategy. (4) After the most concrete strategy only, add ONE inline illustration — 'So if [specific trigger relevant to this company or their clients], I [specific action], which would [directional outcome].' e.g. 'So if a client's CPA rises, I audit targeting changes and form messaging, which would bring conversion costs down.' Do NOT add an illustration after every strategy. (5) Last sentence names the business impact achieved. No full stories. No adjectives. No adverbs. No 'utilize'. Never use vague pronouns — name metrics, channels, tools, and processes explicitly.>\n\
---\n\
...
- Answer: add ONE inline illustration total — after the most concrete strategy only. Do NOT add an illustration after every strategy.\n\
- Answer: inline illustrations must describe directional improvement only — never name a specific metric value or percentage. Use direction (e.g. 'which would bring conversion costs down', 'which reduces wasted spend') — never a fabricated number (e.g. 'by 30%').\n\
