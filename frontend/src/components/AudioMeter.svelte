<script lang="ts">
  const { micLevel, systemLevel, capturing = false } = $props<{
    micLevel: number;
    systemLevel: number;
    capturing?: boolean;
  }>();
  ...
  const sysWarn = $derived(capturing && sysPct < 2);
</script>

<div class="meter" title="Audio input levels — MIC is your microphone, SYS is system audio (interviewer's voice via screen share)">
